              ...
              <div className="issues-listing__header">
                <div className="issues-listing__states">
                  <button className={`btn-link ${this.state.tab === 'open' ? 'btn-link--selected' : ''}`} type="button" onClick={this.openState("open")}>
                    ...
                    {this.state.newIssues.reduce((acc, issue) => (issue.state === "open" ? acc + 1 : acc), 0)} Open
                  </button>
                  <button className={`btn-link ${this.state.tab === "closed" ? "btn-link--selected" : ''}`} type="button" onClick={this.openState("closed")}>
                    ...
                    {this.state.newIssues.reduce((acc, issue) => (issue.state === 'closed' ? acc + 1 : acc), 0)} Closed
                  </button>
                </div>
              </div>
              <div className="issues-listing__body">
                <ul className="issues">
                  {this.state.newIssues.filter((item) => this.state.tab === item.state).map((item) => <IssueItem state={item.state} title={item.title} 
