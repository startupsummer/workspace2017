...
const IssuesItem = (props) => (
  <div className="issues__item">
    ...
    <div className="issues__title">
      <Link to={`/id:${props.issue.id}`} className="issues__link">{props.issue.title}</Link>
    </div>
    <Button
      text={`${props.issue.state === 'open' ? 'Close' : 'Opened'} issues`}
      eventClick={() => props.chengeIssuesStatus(props.issue.number, props.issue.state === 'open' ? 'closed' : 'open')}
    />
  </div>
);
